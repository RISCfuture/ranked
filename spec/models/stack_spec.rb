...
RSpec.describe Stack, type: :model do
  ...
  describe '#to_param' do
    let(:stack) { FactoryBot.build :stack, id: 12345 }
  ...
  describe '#pairs_order' do
    let(:stack) { FactoryBot.create :stack, card_names: [1, 2, 3, 4].map(&:to_s).join("\n") }
  ...
  describe '#rank' do
    it "should return Elo rankings given an ordered array of match results" do
      stack   = FactoryBot.create(:stack, card_names: (1..9).to_a.join("\n"))
  ...
  context '[validations]' do
    it "should require at least 2 cards" do
      stack = FactoryBot.build(:stack, card_names: "foo")
